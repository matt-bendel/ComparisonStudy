...
class SelectiveSliceData_Train(Dataset):
    ...
    def __init__(self, root, transform, challenge, sample_rate=1, use_top_slices=True, number_of_top_slices=8,
        ...
        self.recons_key = 'reconstruction_rss' if challenge == 'singlecoil' \
...
class SelectiveSliceData_Val(Dataset):
    ...
    def __init__(self, root, transform, challenge, sample_rate=1, use_top_slices=True, number_of_top_slices=8,
        ...
        for fname in f[
                     0:]:  # in the validation data, the 0th file has some wierd name and it gives a file not found error. Hence we start from 1
            kspace = h5py.File(fname, 'r')['kspace'][()]
            with h5py.File(fname, 'r') as data:
                keep_files.append(fname)
...
class SelectiveSliceData(Dataset):
    ...
    def __init__(self, root, transform, challenge, sample_rate=1, use_mid_slices=False, num_mid_slices=8,
                 fat_supress=None, strength_3T=None, restrict_size=False):
        """
        Args:
            root (pathlib.Path): Path to the dataset.
