...
def preprocess(in_path, out_path):
    ...
    for fname in tqdm(list(in_path.glob("*.h5"))):
        new_name = Path(f'/storage/fastMRI_brain/data/matt_preprocessed_data/singlecoil_{args.data_type}/' + fname.name)
        ...
        with h5py.File(fname, "r") as hf:
            ...
            if image.shape[-2] < 320:
                diff = 320 - image.shape[-2]
                image = np.pad(image, pad_width=diff)
